...
class Bezier:
  ...
  def eval(self, t):
    ...
    x = 0.0
    for i in range(0, len(self.p)):
      x = x + com(7,i)*self.p[i]*pow(t,i)*pow(1-t,7-i)
    return x
...
# 3d single bezier piece for x-y-z, includes duration.
class Bezier3D:
  def __init__(self, duration, px, py, pz):
    self.duration = duration
    self.px = Bezier(px)
    self.py = Bezier(py)
    self.pz = Bezier(pz)
  ...
  def eval(self, t):
    pos = None
    pos = np.array([self.px.eval(t), self.py.eval(t), self.pz.eval(t)])
    return pos
...
class ColorTrajectory:
  ...
  def loadcsv(self, filename):
    data = np.loadtxt(filename, delimiter=",", skiprows=1, usecols=range(36))
    self.polynomials = [Polynomial4D(row[0], row[1:9], row[9:17], row[17:25], row[25:33]) for row in data]
    self.duration = np.sum(data[:,0])
    self.color = data[:,33:36]
    self.durations = data[:,0]
...
class BezierTrajectory:
  ...
  def loadcsv(self, filename):
    ...
    self.beziers = [Bezier3D(row[0], row[1:9], row[9:17], row[17:25]) for row in data]
    self.duration = np.sum(data[:,0])

  def savecsv(self, filename):
    data = np.empty((len(self.beziers), 8*3+1))
    for i, p in enumerate(self.beziers):
      data[i,0] = p.duration
      data[i,1:9] = p.px.p
      data[i,9:17] = p.py.p
      data[i,17:25] = p.pz.p
  ...
  def eval(self, t):
    ...
    current_t = 0.0
    for p in self.beziers:
      if t < current_t + p.duration:
        return p.eval((t - current_t)/p.duration)
      current_t = current_t + p.duration
