...
class TimeHelper:
    ...
    def sleep(self, duration, color=[0,0,0]):
        self.visualizer.plot = None
        for t in np.arange(self.t, self.t + duration, self.dt):
            self.visualizer.update(t, self.crazyflies, color)
            if self.output:
            ...
            for cf in self.crazyflies:
                rcd = []
                rcd.append(self.t)
                rcd.append(cf.position()[0])
                rcd.append(cf.position()[1])
                rcd.append(cf.position()[2])
                rcd.append(color[0])
                rcd.append(color[1])
                rcd.append(color[2])
                cf.record.append(rcd)
...
class CrazyflieServer:
    ...
    def startColorTrajectory(self, trajectory, trajectoryId, timescale = 1.0, reverse = False, relative = True, groupMask = 0):
        error = []
        cum_e = 0
        for d in trajectory.durations:
            if cum_e > 0:
                error.append(0.1)
                cum_e += float(math.ceil(d*10)/10 - d - 0.1)
            else:
                error.append(0)
                cum_e += float(math.ceil(d*10)/10 - d)
        ...
        for i in range(0,len(trajectory.polynomials)):
            if reverse is False:
                self.timeHelper.sleep(trajectory.durations[i]*timescale-error[i], trajectory.color[i])
            elif reverse is True:
                self.timeHelper.sleep(trajectory.durations[len(trajectory.polynomials)-1-i]*timescale-error[i], trajectory.color[len(trajectory.polynomials)-1-i])
